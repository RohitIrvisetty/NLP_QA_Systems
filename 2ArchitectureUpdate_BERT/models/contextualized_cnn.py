...
from augment.utils import mask_questions_batch
from augment.models.cnn_keras import CNNKerasConfig, apply_conv_layers

# Define the ContextualizedCNNConfig class
class ContextualizedCNNConfig:
    ...
    def _validate(self):
        ...
        assert (self.filter_generator.channels_out[0] %
                self.cnn_downsize.channels_out[-1]) == 0
...
def apply_per_sample_conv1d(a, filters, n_filters, batch_size):
    ...
    assert filters.shape[2] == channels_in * n_filters

    # Reshape and transpose the filters
    filters = tf.reshape(
        filters, [batch_size, filter_length, channels_in, n_filters])

    filters = tf.transpose(filters, [1, 0, 3, 2])
    filters = tf.reshape(
        filters, [filter_length, 1, channels_in * batch_size, n_filters])

    # Apply depthwise convolution
    result = tf.nn.depthwise_conv2d(
        a, filters, strides=[1, 1, 1, 1], padding='SAME')
    result = tf.reshape(
        result, [seq_length, batch_size, channels_in, n_filters])
...
def create_contextualized_cnn_model(is_training,
    ...
    if config.mask_questions:
        paragraphs = mask_questions_batch(
            downsized_input, segment_ids, downsized_channels_out)
    ...
    if config.filter_generator_pooling['type'] == 'max':
        filters = tf.nn.max_pool1d(
            filters, pooling_size, pooling_strides, 'VALID')
    elif config.filter_generator_pooling['type'] == 'avg':
        filters = tf.nn.avg_pool1d(
            filters, pooling_size, pooling_strides, 'VALID')

    contextualized = apply_per_sample_conv1d(
        paragraphs, filters, n_filters, batch_size)
    ...
    def compute_logits(features):
        wd1 = tf.Variable(tf.truncated_normal(
            [feature_channels_out, 1], stddev=0.03), name='wd1')
